...
pub async fn execute_runbook(
    ...
    let mut runbooks = read_runbooks_from_manifest(&manifest, &Some("localnet".into()), Some(&runbook_selector))?;
    let top_level_inputs_map = manifest.get_runbook_inputs(&Some("localnet".into()), &vec![], None)?;
