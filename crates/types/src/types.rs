...
pub enum DataIndexingCommand {
    ...
    ProcessCollectionEntriesPack(Uuid, CollectionEntriesPack),
}

#[derive(Debug, Clone, Deserialize, Serialize)]
pub enum CollectionEntriesPack {
    CpiEvent(CpiEventEntry),
    Pda(PdaEntry),
}
impl CollectionEntriesPack {
...
impl PdaTableDefaults {
    ...
    pub fn columns_with_descriptions() -> Vec<(String, String)> {
        vec![
            ("id".into(), "The UUID of the entry".into()),
            (
                "slot".into(),
...
impl CpiEventTableDefaults {
    pub fn columns() -> Vec<String> {
        vec!["id".into(), "slot".into(), "transactionSignature".into()]
    }
...
#[derive(Debug, Clone)]
pub enum CollectionEntryDataTableDefaults {
    CpiEvent(CpiEventTableDefaults),
    Pda(PdaTableDefaults),
}

impl CollectionEntryDataTableDefaults {
    pub fn default_columns(&self) -> Vec<String> {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(_) => CpiEventTableDefaults::columns(),
            CollectionEntryDataTableDefaults::Pda(_) => PdaTableDefaults::columns(),
        }
    }
    pub fn default_values(&self, uuid: String) -> Vec<String> {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(defaults) => defaults.default_values(uuid),
            CollectionEntryDataTableDefaults::Pda(defaults) => defaults.default_values(uuid),
        }
    }
    pub fn slot(&self) -> u64 {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(defaults) => defaults.slot,
            CollectionEntryDataTableDefaults::Pda(defaults) => defaults.slot,
        }
    }
    pub fn transaction_signature(&self) -> Signature {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(defaults) => defaults.transaction_signature,
            CollectionEntryDataTableDefaults::Pda(defaults) => defaults.transaction_signature,
        }
    }
    pub fn pubkey(&self) -> Option<Pubkey> {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(_) => None,
            CollectionEntryDataTableDefaults::Pda(defaults) => Some(defaults.pubkey),
        }
    }
    pub fn owner(&self) -> Option<Pubkey> {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(_) => None,
            CollectionEntryDataTableDefaults::Pda(defaults) => Some(defaults.owner),
        }
    }
    ...
    pub fn lamports(&self) -> Option<u64> {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(_) => None,
            CollectionEntryDataTableDefaults::Pda(defaults) => Some(defaults.lamports),
        }
    }
    ...
    pub fn write_version(&self) -> Option<u64> {
        match self {
            CollectionEntryDataTableDefaults::CpiEvent(_) => None,
            CollectionEntryDataTableDefaults::Pda(defaults) => Some(defaults.write_version),
        }
    }
}
