...
mod instrumented {
    ...
    // Initialize Prometheus metrics endpoint on bind_addr (e.g., "0.0.0.0:9000")
    pub fn init_prometheus(
        ...
        INIT.call_once(|| {
            // CRITICAL: Use prometheus::Registry::new() - this is what opentelemetry-prometheus expects
            let registry = prometheus::Registry::new();

            // Build the exporter - it registers collectors with the registry
            let exporter = match opentelemetry_prometheus::exporter()
                .with_registry(registry.clone()) // This clones the Arc internally
                .build()
            {
            ...
            let provider = SdkMeterProvider::builder()
                ...
                .with_reader(exporter) // The exporter holds a reference to the registry
                .build();
            ...
            // Spawn HTTP server - use the SAME registry instance
            std::thread::spawn(move || {
                ...
                rt.block_on(async {
                    ...
                    log::info!(
                        "✅ Prometheus metrics on http://{}/metrics",
                        bind_addr_owned
                    );
    ...
    pub fn metrics() -> &'static SurfpoolMetrics {
        METRICS.get().expect("telemetry not initialized")
    }
    ...
    pub fn shutdown() {
        ...
        // OnceLock doesn't have a take() method, but we can leak the memory
